...
export default function TwitterSheet() {
    ...
    useSEO({
        title: 'Twitter (X) Interview Questions & DSA Prep Guide | Whizan AI',
        description: 'Comprehensive list of Twitter (X) software engineer interview questions. Master Twitter Data Structures and Algorithms syllabus, practice exact Twitter coding challenges.',
        canonical: '/company/twitter',
        keywords: 'Twitter software engineer interview questions, Twitter DSA prep, Twitter leetcode questions, Twitter interview roadmap, Twitter technical coding',
        jsonLd: {
            "@context": "https://schema.org",
            "@type": "Course",
            "name": "Twitter Technical Interview Preparation",
            "description": "Specific preparation guide for software engineering technical interviews at Twitter (X), focusing on high-performing DSA.",
            "provider": {
                "@type": "Organization",
                "name": "Whizan AI",
                "sameAs": "https://whizan.xyz"
            },
            "url": "https://whizan.xyz/company/twitter"
        }
    });
    ...
    const { data: problemsData, isLoading: loading } = useQuery({
        queryKey: ['problems-twitter', problemParams],
    ...
    const handlePageChange = (n) => { 
        if (n >= 1 && n <= totalPages) navigate(`/company/twitter/${n}`); 
    };
    ...
    return (
        <div style={{
            minHeight: '100vh',
            background: '#050505',
            backgroundImage: 'radial-gradient(circle at 50% 0%, rgba(29, 161, 242, 0.08) 0%, transparent 60%), radial-gradient(circle at 100% 100%, rgba(29, 161, 242, 0.04) 0%, transparent 50%)',
            color: '#fff',
            ...
            <div className="sheet-content" style={{ maxWidth: '1200px', margin: '0 auto', padding: '3rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
                ...
                <div style={{ textAlign: 'center', marginBottom: '1rem', position: 'relative' }}>
                    <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '300px', height: '300px', background: 'radial-gradient(circle, rgba(29, 161, 242, 0.2) 0%, transparent 70%)', filter: 'blur(40px)', zIndex: 0, pointerEvents: 'none' }} />
                    <div style={{ position: 'relative', zIndex: 1 }}>
                        ...
                        <p className="hero-subtitle">
                            Master Twitter's absolute best and most frequently asked Data Structures and Algorithms questions for software engineer loops.
                        </p>
                    </div>
                </div>
                ...
                <div className="company-sheet-layout">
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center', position: 'relative', zIndex: 10 }}>
                            <div style={{ flex: '1 1 300px', maxWidth: '100%' }}>
                                <Select 
                                    ...
                                    onChange={sel => { setSelectedTopics(sel || []); navigate('/company/twitter/1'); }}
                        ...
                        <div className="problems-container">
                            ...
                            {loading ? (
                                <div style={{ padding: '5rem', textAlign: 'center', color: 'var(--txt3)', fontSize: '0.95rem' }}>
                                    <Zap size={28} style={{ marginBottom: '12px', opacity: 0.5, animation: 'pulse 1.5s infinite' }} /><br />Loading Twitter questions…
                                </div>
                            ) : problems.length === 0 ? (
                                <div style={{ padding: '5rem', textAlign: 'center', color: 'var(--txt3)', fontSize: '0.95rem' }}>No Twitter questions found.</div>
                            ) : problems.map((p, idx) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }} onClick={e => e.stopPropagation()}>
                                            <button onClick={() => handleSolve(p)} style={{ background: 'linear-gradient(135deg, #1DA1F2, #3b82f6)', color: '#fff', border: 'none', borderRadius: '10px', padding: '8px 16px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px', boxShadow: '0 4px 10px rgba(29, 161, 242, 0.3)', transition: 'transform 0.15s' }} onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-2px)'} onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}>Solve <ArrowRight size={14} /></button>
                                        </div>
                                    </div>
                                );
                            })}
                        ...
                        <div style={{ marginTop: '2rem', padding: '2.5rem', background: 'linear-gradient(180deg, rgba(255,255,255,0.01) 0%, rgba(20,22,30,0.4) 100%)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '24px' }}>
                            <h2 style={{ fontSize: '1.8rem', fontWeight: 800, marginBottom: '1.5rem', color: '#fff', letterSpacing: '-0.5px' }}>Master Twitter (X) Software Engineer Interview Questions</h2>
                            <div style={{ color: 'var(--txt2)', fontSize: '0.95rem', lineHeight: 1.7, display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
                                <p>Success at <strong>Twitter technical interviews</strong> requires speed and optimality. Twitter engineers evaluate your ability to think around large-scale data structures and graph-based problem sets common in real-time messaging and social graphs.</p>
                                <h3 style={{ fontSize: '1.2rem', fontWeight: 700, color: '#fff' }}>Twitter Interview Preparation Roadmap</h3>
                                <p>Preparation for <strong>Twitter Leetcode questions</strong> should center on dynamic programming, hash maps, and sliding window techniques. Twitter interviewers often ask variants of problems involving social graphs and real-time data processing. Ensure you optimize your solutions for time-complexity as Twitter focuses heavily on engineering efficiency.</p>
                                <ul style={{ paddingLeft: '1.5rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                                    <li>Focus on real-time architecture—how your DSA solution fits into a larger Twitter-scale system.</li>
                                    <li>Be prepared for iterative follow-ups—Twitter interviewers often tweak problem constraints.</li>
...
const selectStyles = {
    control: (b, s) => ({ ...b, background: 'rgba(255,255,255,0.04)', borderColor: s.isFocused ? '#1DA1F2' : 'rgba(255,255,255,0.15)', borderRadius: '12px', minHeight: '48px', boxShadow: s.isFocused ? '0 0 0 1px #1DA1F2' : 'none' }),
    ...
    option: (b, s) => ({ ...b, background: s.isFocused ? 'rgba(29, 161, 242, 0.15)' : 'transparent', color: s.isFocused ? '#e0f2fe' : '#ccc', cursor: 'pointer', fontSize: '0.9rem', padding: '10px 16px' }),
    multiValue: b => ({ ...b, backgroundColor: 'rgba(29, 161, 242, 0.15)', border: '1px solid rgba(29, 161, 242, 0.3)', borderRadius: '8px' }),
    multiValueLabel: b => ({ ...b, color: '#e0f2fe', fontSize: '0.85rem' }),
    multiValueRemove: b => ({ ...b, color: '#1DA1F2', cursor: 'pointer' }),
    input: b => ({ ...b, color: '#fff' }),
