...
export default function SalesforceSheet() {
    ...
    useSEO({
        title: 'Salesforce Interview Questions & DSA Prep Roadmap | Whizan AI',
        description: 'Prepare for Salesforce software engineering interviews with curated Salesforce Leetcode questions. Master Salesforce DSA syllabus, cloud-native architecture, and CRM system challenges.',
        canonical: '/company/salesforce',
        keywords: 'Salesforce software engineer interview questions, Salesforce DSA prep, Salesforce leetcode questions, Salesforce SDE roadmap, Salesforce technical coding rounds',
        jsonLd: {
            "@context": "https://schema.org",
            "@type": "Course",
            "name": "Salesforce Technical Interview Preparation",
            "description": "Comprehensive roadmap for mastering Data Structures and Algorithms specifically for Salesforce technical interviews.",
            "provider": {
                "@type": "Organization",
                "name": "Whizan AI",
                "sameAs": "https://whizan.xyz"
            },
            "url": "https://whizan.xyz/company/salesforce"
        }
    });
    ...
    const { data: problemsData, isLoading: loading } = useQuery({
        queryKey: ['problems-salesforce', problemParams],
    ...
    const handlePageChange = (n) => { 
        if (n >= 1 && n <= totalPages) navigate(`/company/salesforce/${n}`); 
    };
    ...
    return (
        <div style={{
            minHeight: '100vh',
            background: '#050505',
            backgroundImage: 'radial-gradient(circle at 50% 0%, rgba(0, 161, 224, 0.08) 0%, transparent 60%), radial-gradient(circle at 100% 100%, rgba(0, 161, 224, 0.04) 0%, transparent 50%)',
            color: '#fff',
            ...
            <div className="sheet-content" style={{ maxWidth: '1200px', margin: '0 auto', padding: '3rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
                ...
                <div style={{ textAlign: 'center', marginBottom: '1rem', position: 'relative' }}>
                    <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '300px', height: '300px', background: 'radial-gradient(circle, rgba(0, 161, 224, 0.2) 0%, transparent 70%)', filter: 'blur(40px)', zIndex: 0, pointerEvents: 'none' }} />
                    <div style={{ position: 'relative', zIndex: 1 }}>
                        <div style={{ 
                            display: 'inline-block', padding: '6px 16px', borderRadius: '20px', 
                            background: 'rgba(0, 161, 224, 0.15)', border: '1px solid rgba(0, 161, 224, 0.4)',
                        ...
                        <h1 className="hero-title">
                            <span style={{ color: '#00A1E0' }}>Salesforce</span>
                            <span style={{ color: '#fff' }}>{' '}Interview Questions</span>
                        </h1>
                        <p className="hero-subtitle">
                            Master the most frequently asked Data Structures and Algorithms questions for Salesforce SDE interviews.
                        </p>
                    </div>
                </div>
                ...
                <div className="company-sheet-layout">
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center', position: 'relative', zIndex: 10 }}>
                            <div style={{ flex: '1 1 300px', maxWidth: '100%' }}>
                                <Select 
                                    ...
                                    onChange={sel => { setSelectedTopics(sel || []); navigate('/company/salesforce/1'); }}
                        ...
                        <div className="problems-container">
                            ...
                            {loading ? (
                                <div style={{ padding: '5rem', textAlign: 'center', color: 'var(--txt3)', fontSize: '0.95rem' }}>
                                    <Zap size={28} style={{ marginBottom: '12px', opacity: 0.5, animation: 'pulse 1.5s infinite' }} /><br />Loading Salesforce questions…
                                </div>
                            ) : problems.length === 0 ? (
                                <div style={{ padding: '5rem', textAlign: 'center', color: 'var(--txt3)', fontSize: '0.95rem' }}>No Salesforce questions found.</div>
                            ) : problems.map((p, idx) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }} onClick={e => e.stopPropagation()}>
                                            <button onClick={() => handleSolve(p)} style={{ background: 'linear-gradient(135deg, #00A1E0, #38bdf8)', color: '#fff', border: 'none', borderRadius: '10px', padding: '8px 16px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px', boxShadow: '0 4px 10px rgba(0, 161, 224, 0.3)', transition: 'transform 0.15s' }} onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-2px)'} onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}>Solve <ArrowRight size={14} /></button>
                                        </div>
                                    </div>
                                );
                            })}
                        ...
                        <div style={{ marginTop: '2rem', padding: '2.5rem', background: 'linear-gradient(180deg, rgba(255,255,255,0.01) 0%, rgba(20,22,30,0.4) 100%)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '24px' }}>
                            <h2 style={{ fontSize: '1.8rem', fontWeight: 800, marginBottom: '1.5rem', color: '#fff', letterSpacing: '-0.5px' }}>Master Salesforce Software Engineer Interview Questions</h2>
                            <div style={{ color: 'var(--txt2)', fontSize: '0.95rem', lineHeight: 1.7, display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
                                <p>Preparation for <strong>Salesforce technical interviews</strong> focuses on cloud-native software architecture, multi-tenant databases, and highly optimized DSA solutions. Salesforce interviewers evaluate your ability to handle complex algorithmic challenges related to CRM systems and global data synchronization.</p>
                                <h3 style={{ fontSize: '1.2rem', fontWeight: 700, color: '#fff' }}>Salesforce DSA Preparation strategy</h3>
                                <p>Expect a heavy focus on <strong>Arrays, Trees, and Linked Lists</strong>. Salesforce often uses scenarios involving data migration or user permission logic to frame their technical challenges. Solving <strong>Salesforce Leetcode questions</strong> daily is the most efficient way to scale your problem-solving capabilities.</p>
                                <ul style={{ paddingLeft: '1.5rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                                    <li>Focus on multi-tenant architecture—how your DSA solution fits into a larger Salesforce-scale system.</li>
                                    <li>Be prepared for iterative follow-ups—Salesforce interviewers often tweak problem constraints.</li>
...
const selectStyles = {
    control: (b, s) => ({ ...b, background: 'rgba(255,255,255,0.04)', borderColor: s.isFocused ? '#00A1E0' : 'rgba(255,255,255,0.15)', borderRadius: '12px', minHeight: '48px', boxShadow: s.isFocused ? '0 0 0 1px #00A1E0' : 'none' }),
    ...
    option: (b, s) => ({ ...b, background: s.isFocused ? 'rgba(0, 161, 224, 0.15)' : 'transparent', color: s.isFocused ? '#e0f2fe' : '#ccc', cursor: 'pointer', fontSize: '0.9rem', padding: '10px 16px' }),
    multiValue: b => ({ ...b, backgroundColor: 'rgba(0, 161, 224, 0.15)', border: '1px solid rgba(0, 161, 224, 0.3)', borderRadius: '8px' }),
    multiValueLabel: b => ({ ...b, color: '#e0f2fe', fontSize: '0.85rem' }),
    multiValueRemove: b => ({ ...b, color: '#00A1E0', cursor: 'pointer' }),
    input: b => ({ ...b, color: '#fff' }),
