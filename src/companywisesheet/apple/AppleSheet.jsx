...
export default function AppleSheet() {
    ...
    useSEO({
        title: 'Apple Interview Questions & Technical Prep | Whizan AI',
        description: 'Exhaustive list of Apple technical interview questions. Master Apple Data Structures and Algorithms syllabus, practice exact Apple coding loops.',
        canonical: '/company/apple',
        keywords: 'Apple software engineer interview questions, Apple DSA prep, Apple leetcode questions, Apple interview roadmap, Apple technical coding',
        jsonLd: {
            "@context": "https://schema.org",
            "@type": "Course",
            "name": "Apple Technical Interview Preparation",
            "description": "Vetted list of DSA problems commonly asked in Apple hardware and software engineering interviews.",
            "provider": {
                "@type": "Organization",
                "name": "Whizan AI",
                "sameAs": "https://whizan.xyz"
            },
            "url": "https://whizan.xyz/company/apple"
        }
    });
    ...
    const { data: problemsData, isLoading: loading } = useQuery({
        queryKey: ['problems-apple', problemParams],
    ...
    const handlePageChange = (n) => { 
        if (n >= 1 && n <= totalPages) navigate(`/company/apple/${n}`); 
    };
    ...
    return (
        <div style={{
            minHeight: '100vh',
            background: '#050505',
            backgroundImage: 'radial-gradient(circle at 50% 0%, rgba(162, 170, 173, 0.08) 0%, transparent 60%), radial-gradient(circle at 100% 100%, rgba(162, 170, 173, 0.04) 0%, transparent 50%)',
            color: '#fff',
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '3rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
                ...
                <div style={{ textAlign: 'center', marginBottom: '1rem', position: 'relative' }}>
                    <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '300px', height: '300px', background: 'radial-gradient(circle, rgba(162, 170, 173, 0.2) 0%, transparent 70%)', filter: 'blur(40px)', zIndex: 0, pointerEvents: 'none' }} />
                    <div style={{ position: 'relative', zIndex: 1 }}>
                        <div style={{ 
                            display: 'inline-block', padding: '6px 16px', borderRadius: '20px', 
                            background: 'rgba(162, 170, 173, 0.15)', border: '1px solid rgba(162, 170, 173, 0.4)',
                            color: '#e5e7eb', fontSize: '0.85rem', fontWeight: 700, marginBottom: '20px',
                            boxShadow: '0 4px 14px rgba(162, 170, 173, 0.2)'
                        }}>
                            <Building2 size={14} style={{ display: 'inline', marginRight: '6px', verticalAlign: 'middle', marginTop: '-2px' }}/>
                            Vetted Apple Roadmap
                        </div>
                        <h1 style={{ fontSize: '3rem', fontWeight: 800, margin: '0 0 16px 0', letterSpacing: '-1.5px', textShadow: '0 2px 10px rgba(0,0,0,0.5)' }}>
                            ...
                            <span style={{ color: '#666' }}>{' '}Interview Questions</span>
                        </h1>
                        <p style={{ color: 'var(--txt2)', fontSize: '1.05rem', maxWidth: '650px', margin: '0 auto', lineHeight: 1.6 }}>
                            Master the absolute best and most frequently asked Data Structures and Algorithms questions for Apple technical interviews.
                        </p>
                    </div>
                </div>
                ...
                <div className="company-sheet-layout">
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center', position: 'relative', zIndex: 10 }}>
                            <div style={{ flex: '1 1 300px', maxWidth: '100%' }}>
                                <Select 
                                    ...
                                    onChange={sel => { setSelectedTopics(sel || []); navigate('/company/apple/1'); }}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0', background: 'rgba(15,15,20,0.6)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 10px 40px rgba(0,0,0,0.2)' }}>
                            ...
                            {loading ? (
                                <div style={{ padding: '5rem', textAlign: 'center', color: 'var(--txt3)', fontSize: '0.95rem' }}>
                                    <Zap size={28} style={{ marginBottom: '12px', opacity: 0.5, animation: 'pulse 1.5s infinite' }} /><br />Loading Apple questions…
                                </div>
                            ) : problems.length === 0 ? (
                                <div style={{ padding: '5rem', textAlign: 'center', color: 'var(--txt3)', fontSize: '0.95rem' }}>No Apple questions found.</div>
                            ) : problems.map((p, idx) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }} onClick={e => e.stopPropagation()}>
                                            <button onClick={() => handleSolve(p)} style={{ background: 'linear-gradient(135deg, #A2AAAD, #fff)', color: '#000', border: 'none', borderRadius: '10px', padding: '8px 16px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '6px', boxShadow: '0 4px 10px rgba(162, 170, 173, 0.3)', transition: 'transform 0.15s' }} onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-2px)'} onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}>Solve <ArrowRight size={14} /></button>
                                        </div>
                                    </div>
                                );
                            })}
                        ...
                        <div style={{ marginTop: '2rem', padding: '2.5rem', background: 'linear-gradient(180deg, rgba(255,255,255,0.01) 0%, rgba(20,22,30,0.4) 100%)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '24px' }}>
                            <h2 style={{ fontSize: '1.8rem', fontWeight: 800, marginBottom: '1.5rem', color: '#fff', letterSpacing: '-0.5px' }}>Master Apple Software Engineer Interview Questions</h2>
                            <div style={{ color: 'var(--txt2)', fontSize: '0.95rem', lineHeight: 1.7, display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
                                <p>Preparation for <strong>Apple software engineering interviews</strong> often focuses on low-level system design, memory management, and highly optimized DSA solutions. Our curated <strong>Apple technical questions</strong> cover the full spectrum of their internal hardware and software engineering loops.</p>
                                <h3 style={{ fontSize: '1.2rem', fontWeight: 700, color: '#fff' }}>Apple DSA Preparation Strategy</h3>
                                <p>Apple engineers prioritize resource efficiency. When solving <strong>Apple Leetcode questions</strong>, pay close attention to space complexity and bit manipulation. Common topics include stack-based challenges, recursive data structures, and optimized search algorithms for large datasets.</p>
                                <ul style={{ paddingLeft: '1.5rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                                    <li>Optimize for performance—Apple software must run smoothly on highly integrated systems.</li>
                                    <li>Revise core networking and operating system concepts if targeting embedded or OS teams.</li>
                                    <li>Focus on clean, highly readable source code that demonstrates mature engineering principles.</li>
                                </ul>
                            </div>
                        </div>
...
const selectStyles = {
    control: (b, s) => ({ ...b, background: 'rgba(255,255,255,0.04)', borderColor: s.isFocused ? '#A2AAAD' : 'rgba(255,255,255,0.15)', borderRadius: '12px', minHeight: '48px', boxShadow: s.isFocused ? '0 0 0 1px #A2AAAD' : 'none' }),
    ...
    option: (b, s) => ({ ...b, background: s.isFocused ? 'rgba(162,170,173,0.15)' : 'transparent', color: s.isFocused ? '#fff' : '#ccc', cursor: 'pointer', fontSize: '0.9rem', padding: '10px 16px' }),
    multiValue: b => ({ ...b, backgroundColor: 'rgba(162,170,173,0.15)', border: '1px solid rgba(162,170,173,0.3)', borderRadius: '8px' }),
    multiValueLabel: b => ({ ...b, color: '#fff', fontSize: '0.85rem' }),
    multiValueRemove: b => ({ ...b, color: '#A2AAAD', cursor: 'pointer' }),
    input: b => ({ ...b, color: '#fff' }),
