...
export default function JPMorganSheet() {
    ...
    useSEO({
        title: 'JPMorgan Chase Interview Questions & Technical Prep | Whizan AI',
        description: 'Vetted list of JPMorgan Chase software engineer interview questions. Master JPMC technical rounds with curated Leetcode DSA problems.',
        canonical: '/company/jpmorgan',
        keywords: 'JPMorgan Chase software engineer interview questions, JPMC DSA prep, JPMC leetcode questions, JPMorgan roadmap, technical coding',
        jsonLd: {
            "@context": "https://schema.org",
            "@type": "Course",
            "name": "JPMorgan Chase Technical Interview Preparation",
            "description": "Specific preparation roadmap for mastering Data Structures and Algorithms for JPMorgan Chase software engineering roles.",
            "provider": { "@type": "Organization", "name": "Whizan AI", "sameAs": "https://whizan.xyz" },
            "url": "https://whizan.xyz/company/jpmorgan"
        }
    });
    ...
    const problemParams = { page, search: '', topics: selectedTopics.map(t => t.value), companies: ['JPMorgan'] };
    const { data: problemsData, isLoading: loading } = useQuery({
        queryKey: ['problems-jpmorgan', problemParams],
    ...
    const handlePageChange = (n) => { if (n >= 1 && n <= totalPages) navigate(`/company/jpmorgan/${n}`); };

    return (
        <div style={{
            minHeight: '100vh', background: '#050505',
            backgroundImage: 'radial-gradient(circle at 50% 0%, rgba(27, 20, 100, 0.08) 0%, transparent 60%), radial-gradient(circle at 100% 100%, rgba(27, 20, 100, 0.04) 0%, transparent 50%)',
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '3rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
                <div style={{ textAlign: 'center', marginBottom: '1rem', position: 'relative' }}>
                    <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '300px', height: '300px', background: 'radial-gradient(circle, rgba(27, 20, 100, 0.2) 0%, transparent 70%)', filter: 'blur(40px)', zIndex: 0, pointerEvents: 'none' }} />
                    <div style={{ position: 'relative', zIndex: 1 }}>
                        <div style={{ 
                            display: 'inline-block', padding: '6px 16px', borderRadius: '20px', 
                            background: 'rgba(27, 20, 100, 0.15)', border: '1px solid rgba(27, 20, 100, 0.4)',
                            color: '#dbeafe', fontSize: '0.85rem', fontWeight: 700, marginBottom: '20px'
                        }}>
                            <Building2 size={14} style={{ display: 'inline', marginRight: '6px', verticalAlign: 'middle' }}/> Premium JPMorgan Prep
                        </div>
                        <h1 style={{ fontSize: '3rem', fontWeight: 800, margin: '0 0 16px 0', letterSpacing: '-1.5px' }}>
                            <span style={{ color: '#1B1464' }}>JPMorgan Chase</span> Interview Questions
                        </h1>
                        <p style={{ color: '#94a3b8', fontSize: '1.05rem', maxWidth: '650px', margin: '0 auto', lineHeight: 1.6 }}>
                            Excel in JPMorgan technical rounds with our handpicked collection of Data Structures and Algorithms challenges.
                        </p>
                    </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 350px', gap: '2.5rem', alignItems: 'start' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                        <Select isMulti options={metadata.topics} value={selectedTopics} onChange={sel => { setSelectedTopics(sel || []); navigate('/company/jpmorgan/1'); }} placeholder="Filter by Topics…" styles={selectStyles} />
                        <div style={{ background: 'rgba(15,15,20,0.6)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '20px', overflow: 'hidden' }}>
                            ...
                            {loading ? <div style={{ padding: '5rem', textAlign: 'center' }}>Loading JPMorgan questions…</div> : problems.map((p, idx) => (
                                <div key={p.id} onClick={() => handleSolve(p)} style={{ display: 'grid', gridTemplateColumns: '56px 1fr 100px 90px 110px', alignItems: 'center', padding: '18px 24px', borderBottom: idx < problems.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none', cursor: 'pointer' }}>
                                    ...
                                    <button style={{ background: '#1B1464', color: '#fff', border: 'none', borderRadius: '8px', padding: '6px 12px', cursor: 'pointer' }}>Solve</button>
                                </div>
                            ))}
                        </div>
                        <CompanyGrid />
...
const selectStyles = {
    control: (b, s) => ({ ...b, background: 'rgba(255,255,255,0.04)', borderColor: s.isFocused ? '#1B1464' : 'rgba(255,255,255,0.15)', borderRadius: '12px' }),
