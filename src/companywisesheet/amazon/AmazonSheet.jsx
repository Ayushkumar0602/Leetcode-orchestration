...
export default function AmazonSheet() {
    ...
    useSEO({
        title: 'Amazon Interview Questions & DSA Preparation Roadmap | Whizan AI',
        description: 'Prepare for Amazon SDE interviews with curated Amazon Leetcode questions. Master Amazon DSA syllabus, AWS concepts, and typical software engineer coding challenges.',
        canonical: '/company/amazon',
        keywords: 'Amazon software engineer interview questions, Amazon DSA prep, Amazon leetcode questions, Amazon SDE roadmap, AWS engineer interview, Amazon coding challenges',
        jsonLd: {
            "@context": "https://schema.org",
            "@type": "Course",
            "name": "Amazon Technical Interview Preparation",
            "description": "Premium roadmap for mastering Data Structures and Algorithms specifically for Amazon technical interviews.",
            "provider": {
                "@type": "Organization",
                "name": "Whizan AI",
                "sameAs": "https://whizan.xyz"
            },
            "url": "https://whizan.xyz/company/amazon"
        }
    });
    ...
    const { data: problemsData, isLoading: loading } = useQuery({
        queryKey: ['problems-amazon', problemParams],
    ...
    const handlePageChange = (n) => { 
        if (n >= 1 && n <= totalPages) navigate(`/company/amazon/${n}`); 
    };
    ...
    return (
        <div style={{
            minHeight: '100vh',
            background: '#050505',
            backgroundImage: 'radial-gradient(circle at 50% 0%, rgba(255, 153, 0, 0.08) 0%, transparent 60%), radial-gradient(circle at 100% 100%, rgba(255, 153, 0, 0.04) 0%, transparent 50%)',
            color: '#fff',
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '3rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
                ...
                <div style={{ textAlign: 'center', marginBottom: '1rem', position: 'relative' }}>
                    <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '300px', height: '300px', background: 'radial-gradient(circle, rgba(255, 153, 0, 0.2) 0%, transparent 70%)', filter: 'blur(40px)', zIndex: 0, pointerEvents: 'none' }} />
                    <div style={{ position: 'relative', zIndex: 1 }}>
                        ...
                        <p style={{ color: 'var(--txt2)', fontSize: '1.05rem', maxWidth: '650px', margin: '0 auto', lineHeight: 1.6 }}>
                            Master the most frequently asked Data Structures and Algorithms questions for Amazon SDE interviews.
                        </p>
                    </div>
                </div>
                ...
                <div className="company-sheet-layout">
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center', position: 'relative', zIndex: 10 }}>
                            <div style={{ flex: '1 1 300px', maxWidth: '100%' }}>
                                <Select 
                                    ...
                                    onChange={sel => { setSelectedTopics(sel || []); navigate('/company/amazon/1'); }}
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0', background: 'rgba(15,15,20,0.6)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 10px 40px rgba(0,0,0,0.2)' }}>
                            ...
                            {loading ? (
                                <div style={{ padding: '5rem', textAlign: 'center', color: 'var(--txt3)', fontSize: '0.95rem' }}>
                                    <Zap size={28} style={{ marginBottom: '12px', opacity: 0.5, animation: 'pulse 1.5s infinite' }} /><br />Loading Amazon questions…
                                </div>
                            ) : problems.length === 0 ? (
                                <div style={{ padding: '5rem', textAlign: 'center', color: 'var(--txt3)', fontSize: '0.95rem' }}>No Amazon questions found.</div>
                            ) : problems.map((p, idx) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }} onClick={e => e.stopPropagation()}>
                                            <button onClick={() => handleSolve(p)} style={{ background: 'linear-gradient(135deg, #FF9900, #ffb347)', color: '#fff', border: 'none', borderRadius: '10px', padding: '8px 16px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px', boxShadow: '0 4px 10px rgba(255, 153, 0, 0.3)', transition: 'transform 0.15s' }} onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-2px)'} onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}>Solve <ArrowRight size={14} /></button>
                                        </div>
                                    </div>
                                );
                            })}
                        ...
                        <div style={{ marginTop: '2rem', padding: '2.5rem', background: 'linear-gradient(180deg, rgba(255,255,255,0.01) 0%, rgba(20,22,30,0.4) 100%)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '24px' }}>
                            <h2 style={{ fontSize: '1.8rem', fontWeight: 800, marginBottom: '1.5rem', color: '#fff', letterSpacing: '-0.5px' }}>Master Amazon Software Engineer Interview Questions</h2>
                            <div style={{ color: 'var(--txt2)', fontSize: '0.95rem', lineHeight: 1.7, display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
                                <p>Preparing for an <strong>Amazon technical interview</strong> requires a deep understanding of core DSA concepts and <strong>Amazon Leadership Principles</strong>. Our list of <strong>Amazon specific interview questions</strong> is curated from real candidate experiences at AWS and other Amazon teams.</p>
                                <h3 style={{ fontSize: '1.2rem', fontWeight: 700, color: '#fff' }}>Amazon Coding Interview Patterns</h3>
                                <p>Expect a heavy focus on <strong>Arrays, Strings, Linked Lists</strong>, and <strong>Tree traversal</strong>. Amazon often uses scenarios involving order processing or customer behavioral data to frame their technical challenges. Solving <strong>Amazon Leetcode questions</strong> daily is the most efficient way to scale your problem-solving capabilities.</p>
                                <ul style={{ paddingLeft: '1.5rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                                    <li>Focus on time and space complexity optimizations—Amazon engineers value efficient source code.</li>
                                    <li>Practice behavioral questions alongside coding to demonstrate leadership principles.</li>
                                    <li>Master graph algorithms and dynamic programming to tackle advanced SDE2 loops.</li>
                                </ul>
                            </div>
                            <CompanyGrid />
                        </div>
...
const selectStyles = {
    control: (b, s) => ({ ...b, background: 'rgba(255,255,255,0.04)', borderColor: s.isFocused ? '#FF9900' : 'rgba(255,255,255,0.15)', borderRadius: '12px', minHeight: '48px', boxShadow: s.isFocused ? '0 0 0 1px #FF9900' : 'none' }),
    ...
    option: (b, s) => ({ ...b, background: s.isFocused ? 'rgba(255,153,0,0.15)' : 'transparent', color: s.isFocused ? '#fff7ed' : '#ccc', cursor: 'pointer', fontSize: '0.9rem', padding: '10px 16px' }),
    multiValue: b => ({ ...b, backgroundColor: 'rgba(255,153,0,0.15)', border: '1px solid rgba(255,153,0,0.3)', borderRadius: '8px' }),
    multiValueLabel: b => ({ ...b, color: '#fff7ed', fontSize: '0.85rem' }),
    multiValueRemove: b => ({ ...b, color: '#FF9900', cursor: 'pointer' }),
    input: b => ({ ...b, color: '#fff' }),
