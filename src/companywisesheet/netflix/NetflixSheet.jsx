...
export default function NetflixSheet() {
    ...
    useSEO({
        title: 'Netflix Interview Questions & Technical Prep Guide | Whizan AI',
        description: 'Exhaustive list of Netflix technical interview questions. Master Netflix Data Structures and Algorithms syllabus, practice exact Netflix coding challenges.',
        canonical: '/company/netflix',
        keywords: 'Netflix software engineer interview questions, Netflix DSA prep, Netflix leetcode questions, Netflix interview roadmap, Netflix technical coding',
        jsonLd: {
            "@context": "https://schema.org",
            "@type": "Course",
            "name": "Netflix Technical Interview Preparation",
            "description": "Specific preparation guide for software engineering technical interviews at Netflix, focusing on high-performing DSA.",
            "provider": {
                "@type": "Organization",
                "name": "Whizan AI",
                "sameAs": "https://whizan.xyz"
            },
            "url": "https://whizan.xyz/company/netflix"
        }
    });
    ...
    const { data: problemsData, isLoading: loading } = useQuery({
        queryKey: ['problems-netflix', problemParams],
    ...
    const handlePageChange = (n) => { 
        if (n >= 1 && n <= totalPages) navigate(`/company/netflix/${n}`); 
    };
    ...
    return (
        <div style={{
            minHeight: '100vh',
            background: '#050505',
            backgroundImage: 'radial-gradient(circle at 50% 0%, rgba(229, 9, 20, 0.08) 0%, transparent 60%), radial-gradient(circle at 100% 100%, rgba(229, 9, 20, 0.04) 0%, transparent 50%)',
            color: '#fff',
            ...
            <div className="sheet-content" style={{ maxWidth: '1200px', margin: '0 auto', padding: '3rem 1.5rem', display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
                ...
                <div style={{ textAlign: 'center', marginBottom: '1rem', position: 'relative' }}>
                    <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '300px', height: '300px', background: 'radial-gradient(circle, rgba(229, 9, 20, 0.2) 0%, transparent 70%)', filter: 'blur(40px)', zIndex: 0, pointerEvents: 'none' }} />
                    <div style={{ position: 'relative', zIndex: 1 }}>
                        ...
                        <p className="hero-subtitle">
                            Master the most frequently asked Data Structures and Algorithms questions for Netflix technical interviews.
                        </p>
                    </div>
                </div>
                ...
                <div className="company-sheet-layout">
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center', position: 'relative', zIndex: 10 }}>
                            <div style={{ flex: '1 1 300px', maxWidth: '100%' }}>
                                <Select 
                                    ...
                                    onChange={sel => { setSelectedTopics(sel || []); navigate('/company/netflix/1'); }}
                        ...
                        <div className="problems-container">
                            ...
                            {loading ? (
                                <div style={{ padding: '5rem', textAlign: 'center', color: 'var(--txt3)', fontSize: '0.95rem' }}>
                                    <Zap size={28} style={{ marginBottom: '12px', opacity: 0.5, animation: 'pulse 1.5s infinite' }} /><br />Loading Netflix questions…
                                </div>
                            ) : problems.length === 0 ? (
                                <div style={{ padding: '5rem', textAlign: 'center', color: 'var(--txt3)', fontSize: '0.95rem' }}>No Netflix questions found.</div>
                            ) : problems.map((p, idx) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }} onClick={e => e.stopPropagation()}>
                                            <button onClick={() => handleSolve(p)} style={{ background: 'linear-gradient(135deg, #E50914, #f87171)', color: '#fff', border: 'none', borderRadius: '10px', padding: '8px 16px', cursor: 'pointer', fontSize: '0.85rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '6px', boxShadow: '0 4px 10px rgba(229, 9, 20, 0.3)', transition: 'transform 0.15s' }} onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-2px)'} onMouseLeave={e => e.currentTarget.style.transform = 'translateY(0)'}>Solve <ArrowRight size={14} /></button>
                                        </div>
                                    </div>
                                );
                            })}
                        ...
                        <div style={{ marginTop: '2rem', padding: '2.5rem', background: 'linear-gradient(180deg, rgba(255,255,255,0.01) 0%, rgba(20,22,30,0.4) 100%)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '24px' }}>
                            <h2 style={{ fontSize: '1.8rem', fontWeight: 800, marginBottom: '1.5rem', color: '#fff', letterSpacing: '-0.5px' }}>Master Netflix Software Engineer Interview Questions</h2>
                            <div style={{ color: 'var(--txt2)', fontSize: '0.95rem', lineHeight: 1.7, display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
                                <p>Preparation for <strong>Netflix technical interviews</strong> focuses on high-performing, scalable DSA solutions. Netflix is known for its high talent bar and "Freedom & Responsibility" culture, which often translates to more open-ended technical challenges.</p>
                                <h3 style={{ fontSize: '1.2rem', fontWeight: 700, color: '#fff' }}>Netflix technical Interview Patterns</h3>
                                <p>Preparation for <strong>Netflix Leetcode questions</strong> should center on distributed systems logic, large-scale data processing, and highly efficient sorting algorithms. Netflix engineers value clean source code that can handle massive throughput and edge cases common in global streaming architectures.</p>
                                <ul style={{ paddingLeft: '1.5rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                                    <li>Focus on scalability—how your DSA solution handles millions of concurrent data points.</li>
                                    <li>Revise core networking and performance tuning concepts alongside your technical preparation.</li>
                                    <li>Master graph algorithms and dynamic programming to solve complex optimization challenges common in content recommendation systems.</li>
                                </ul>
                            </div>
                        </div>
...
const selectStyles = {
    control: (b, s) => ({ ...b, background: 'rgba(255,255,255,0.04)', borderColor: s.isFocused ? '#E50914' : 'rgba(255,255,255,0.15)', borderRadius: '12px', minHeight: '48px', boxShadow: s.isFocused ? '0 0 0 1px #E50914' : 'none' }),
    ...
    option: (b, s) => ({ ...b, background: s.isFocused ? 'rgba(229,9,20,0.15)' : 'transparent', color: s.isFocused ? '#fee2e2' : '#ccc', cursor: 'pointer', fontSize: '0.9rem', padding: '10px 16px' }),
    multiValue: b => ({ ...b, backgroundColor: 'rgba(229,9,20,0.15)', border: '1px solid rgba(229,9,20,0.3)', borderRadius: '8px' }),
    multiValueLabel: b => ({ ...b, color: '#fee2e2', fontSize: '0.85rem' }),
    multiValueRemove: b => ({ ...b, color: '#E50914', cursor: 'pointer' }),
    input: b => ({ ...b, color: '#fff' }),
