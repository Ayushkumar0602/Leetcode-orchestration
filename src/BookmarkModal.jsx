...
export default function BookmarkModal({ problemId, userId, onClose }) {
    ...
    useEffect(() => {
        if (!userId) return;
        setLoading(true);
        fetch(`https://leetcode-orchestration-api.onrender.com/api/lists/${userId}`)
            .then(r => r.json())
    ...
    const toggleList = async (list) => {
        ...
        try {
            if (inList) {
                await fetch(`https://leetcode-orchestration-api.onrender.com/api/lists/${list.id}/problems/${problemId}`, { method: 'DELETE' });
                setLists(prev => prev.map(l => l.id === list.id ? { ...l, problemIds: l.problemIds.filter(id => id !== String(problemId)) } : l));
            } else {
                await fetch(`https://leetcode-orchestration-api.onrender.com/api/lists/${list.id}/add`, {
                    method: 'POST',
