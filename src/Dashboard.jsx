...
export default function Dashboard() {
    ...
    useEffect(() => {
        const loadProblem = async () => {
            ...
            if (location.state?.problemParams) {
                ...
                if (params.id) {
                    fetch(`https://leetcode-orchestration-api.onrender.com/api/problems/${params.id}`)
                        .then(r => r.json())
                        .then(d => {
            ...
            // Case 2: Direct link visit with an ID in the URL
                ...
                try {
                    ...
                    const res = await fetch(`https://leetcode-orchestration-api.onrender.com/api/problems/${routeId}`);
        ...
        try {
            ...
            if (currentUser) {
                ...
                if (activeProblemId) {
                    ...
                    fetch('https://leetcode-orchestration-api.onrender.com/api/submissions/save', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                ...
                while (boundary !== -1) {
                    ...
                    if (eventString.startsWith('data: ')) {
                        ...
                        if (data.type === 'progress') {
                            ...
                            if (activeProblemId) {
                                fetch('https://leetcode-orchestration-api.onrender.com/api/stats/submit', {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
    ...
    const fetchSubmissionHistory = async () => {
        ...
        try {
            const res = await fetch(`https://leetcode-orchestration-api.onrender.com/api/submissions/${currentUser.uid}/${activeProblemId}`);
            const data = await res.json();
