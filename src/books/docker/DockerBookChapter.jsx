...
// Navigation structure for the Docker Deep Dive series
// ── Part 1: The big picture stuff ──────────────────────────────────────────
const part1 = [
  { id: 'chapter1', title: 'Chapter 1: Containers from 30,000 feet' },
  { id: 'chapter2', title: 'Chapter 2: Docker & container standards' },
  { id: 'chapter3', title: 'Chapter 3: Getting Docker' },
  { id: 'chapter4', title: 'Chapter 4: The big picture' },
];

// ── Part 2: The technical stuff ─────────────────────────────────────────────
const part2 = [
  { id: 'chapter5',  title: 'Chapter 5: The Docker Engine' },
  { id: 'chapter6',  title: 'Chapter 6: Working with Images' },
  { id: 'chapter7',  title: 'Chapter 7: Working with Containers' },
  { id: 'chapter8',  title: 'Chapter 8: Containerizing an App' },
  { id: 'chapter9',  title: 'Chapter 9: Multi-container Apps with Compose' },
  { id: 'chapter10', title: 'Chapter 10: Docker and AI' },
  { id: 'chapter11', title: 'Chapter 11: Docker and Wasm' },
];

const bookChapters = [...part1, ...part2];
...
export default function DockerBookChapter() {
  ...
  const currentChapterInfo = bookChapters.find(c => c.id === topicname) || { title: `Docker - ${topicname}` };
  ...
  useEffect(() => {
    ...
    setBlocks([]);
    window.scrollTo(0, 0);
    fetch(`/books/docker/data/${topicname}.json`)
      .then(r => {
  ...
  return (
    <div className={`reader-body ${theme === 'dark' ? '' : 'theme-' + theme}`}>
      <Helmet>
        <title>{currentChapterInfo.title} | Docker Deep Dive - Whizan AI</title>
        <meta name="description" content={`Master Docker with ${currentChapterInfo.title} from Docker Deep Dive. Understand the big picture of containers, architecture, and deployment.`} />
        <meta name="keywords" content="Docker, Containers, Docker Deep Dive, Nigel Poulton, DevOps, Kubernetes, Software Engineering" />
        <link rel="canonical" href={`https://whizan.xyz/books/docker/${topicname}`} />
        <meta property="og:title" content={`${currentChapterInfo.title} - Docker Deep Dive`} />
        <meta property="og:description" content={`Master Docker with ${currentChapterInfo.title}. A comprehensive guide from zero to Docker.`} />
        <meta property="og:type" content="article" />
        <meta property="og:url" content={`https://whizan.xyz/books/docker/${topicname}`} />
      </Helmet>
      <style>{styles}</style>
      ...
      <div className="reader-container">
        ...
        <aside className={`reader-sidebar ${sidebarOpen ? 'open' : ''} ${!desktopSidebarOpen ? 'collapsed' : ''}`}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {/* Part 1 group */}
            <div style={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-muted)', padding: '8px 18px 4px', opacity: 0.6 }}>
              Part 1 · Big Picture
            </div>
            {part1.map((chapter, index) => (
              <button
                key={chapter.id}
                className={`chapter-link ${topicname === chapter.id ? 'active' : ''}`}
                onClick={() => {
                  navigate(`/books/docker/${chapter.id}`);
            ...
            {/* Part 2 group */}
            <div style={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-muted)', padding: '16px 18px 4px', opacity: 0.6 }}>
              Part 2 · Technical Stuff
            </div>
            {part2.map((chapter, index) => (
              <button
                key={chapter.id}
                className={`chapter-link ${topicname === chapter.id ? 'active' : ''}`}
                onClick={() => {
              ...
              >
                <span className="chapter-number">{(index + 5).toString().padStart(2, '0')}</span>
                <span style={{flex: 1, textAlign: 'left'}}>{chapter.title.split(': ')[1] || chapter.title}</span>
                {topicname === chapter.id && <ChevronRight size={16} />}
              </button>
            ))}
        ...
        <main className="reader-content">
          ...
          {/* Bottom Navigation */}
          {blocks.length > 0 && (
            <div style={{ marginTop: '5rem', paddingTop: '2rem', borderTop: '1px solid var(--border-color)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
               {prevChapter ? (
                 <button onClick={() => navigate(`/books/docker/${prevChapter.id}`)} style={{ background: 'var(--hover-bg)', color: 'var(--text-main)', border: '1px solid var(--border-color)', padding: '12px 24px', borderRadius: '12px', cursor: 'pointer', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '8px', flex: '1 1 auto', justifyContent: 'center' }}>
                   <ArrowLeft size={18} /> Previous
                 </button>
               ) : (
                 <button onClick={() => window.scrollTo(0, 0)} style={{ background: 'var(--hover-bg)', color: 'var(--text-main)', border: 'none', padding: '12px 24px', borderRadius: '12px', cursor: 'pointer', transition: 'background 0.2s', fontWeight: 500, flex: '1 1 auto', justifyContent: 'center' }}>
                   Back to Top
                 </button>
               )}
               {nextChapter && (
                 <button onClick={() => navigate(`/books/docker/${nextChapter.id}`)} style={{ background: 'var(--accent)', color: '#fff', border: 'none', padding: '12px 24px', borderRadius: '12px', cursor: 'pointer', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '8px', flex: '1 1 auto', justifyContent: 'center' }}>
